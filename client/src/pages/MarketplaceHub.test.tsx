...
describe("MarketplaceHub", () => { afterEach(cleanup); it("shows a genuine empty marketplace and does not display fabricated shops, reviews, or payments", () => { render(<LanguageProvider><MarketplaceHub /></LanguageProvider>); expect(screen.getByRole("heading", { name: "السوق ينتظر أول متجر حقيقي" })).toBeTruthy(); expect(screen.getByText(/لا توجد أمثلة وهمية/)).toBeTruthy(); expect(screen.getByText(/لا مدفوعات بعد/)).toBeTruthy(); expect(screen.getByRole("button", { name: "تسجيل الدخول والمتابعة" })).toBeTruthy(); expect(screen.queryByText("★★★★★")).toBeNull(); expect(screen.queryByText(/مراجعة عميل/)).toBeNull(); }); });
