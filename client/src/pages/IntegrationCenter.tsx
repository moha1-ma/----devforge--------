...
export default function IntegrationCenter() {
  ...
  return <DashboardLayout><div dir={direction} className="mx-auto max-w-6xl space-y-5">
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{filtered.map(item => { const preference = requested.get(item.providerKey); return <Card key={item.providerKey} className="flex min-w-0 flex-col border-white/10 bg-slate-950/55 text-white"><CardHeader className="pb-3"><div className="flex items-center justify-between gap-2"><Badge variant="outline" className="border-white/15 text-slate-300">{item.category}</Badge>{preference ? <Badge className="bg-emerald-400/15 text-emerald-200"><CheckCircle2 className="ml-1 h-3.5 w-3.5" />{t("requestRecorded")}</Badge> : <Badge className="bg-slate-400/10 text-slate-300">{t("disconnected")}</Badge>}</div><CardTitle className="mt-3 text-lg">{item.name}</CardTitle><CardDescription className="min-h-12 leading-6 text-slate-400">{item.description}</CardDescription></CardHeader><CardContent className="mt-auto space-y-3">{item.availability ? <Badge className="bg-emerald-300/10 text-emerald-100">{availabilityCopy[item.availability]}</Badge> : null}<p className="rounded-lg border border-white/8 bg-white/4 p-2 text-xs text-slate-300">{methodCopy[item.connectionMethod]} — {t("proposedScope")}: {item.requestedScope}</p><p className="text-xs leading-5 text-amber-100/80">{item.protectedBoundary}</p>{item.documentationUrl ? <a href={item.documentationUrl} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-xs font-semibold text-cyan-200 hover:text-cyan-100">توثيق {item.name}<ExternalLink className="h-3.5 w-3.5" /></a> : null}<Button className="w-full" variant={preference ? "outline" : "default"} disabled={requestConnection.isPending || Boolean(preference)} onClick={() => requestConnection.mutate({ providerKey: item.providerKey })}>{preference ? t("requestRecorded") : t("prepareSecureConnection")}</Button></CardContent></Card>; })}</div>
