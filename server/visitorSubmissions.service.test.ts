...
const values = vi.fn(); const returningId = vi.fn(); const insert = vi.fn(); const getDb = vi.fn(); const storagePut = vi.fn(); const storageGet = vi.fn(); const notifyOwnerOfVisitorSubmission = vi.fn();
vi.mock("./db", () => ({ getDb }));
vi.mock("./storage", () => ({ storageGet, storagePut, storageGetSignedUrl: vi.fn() }));
vi.mock("./siteNotifications", () => ({ notifyOwnerOfVisitorSubmission }));
describe("submitVisitorContribution", () => {
  beforeEach(() => { vi.clearAllMocks(); returningId.mockResolvedValue([{ id: 701 }]); values.mockImplementation(() => ({ $returningId: returningId })); insert.mockReturnValue({ values }); getDb.mockResolvedValue({ insert }); storagePut.mockResolvedValue({ key: "private/visitor/701/example.py" }); notifyOwnerOfVisitorSubmission.mockResolvedValue(null); });
  it("uses the exact insert identifier for attachment storage rather than the newest global submission", async () => {
    ...
    expect(notifyOwnerOfVisitorSubmission).toHaveBeenCalledOnce();
