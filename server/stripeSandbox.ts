export function requireStripeSandboxKey(secretKey: string | undefined): string {
  if (!secretKey) {
...
export function stripeFormBody(values: Record<string, string | number | boolean>): URLSearchParams {
  const body = new URLSearchParams();
  Object.entries(values).forEach(([key, value]) => body.set(key, String(value)));
  return body;
}

export function maskStripeId(value: string | undefined): string {
