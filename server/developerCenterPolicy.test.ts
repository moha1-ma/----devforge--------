...
describe("Developer Center review-only policy", () => {
  ...
  it("accepts only structured review-only proposals", () => {
    const valid = JSON.stringify({ executionStatus: "review-only", headline: "منصة تعليمية", scope: ["لوحة"], architecture: ["واجهة"], websiteOutline: { title: "تعلم", pages: ["الرئيسية"], hierarchy: ["الرئيسية > مسار"], primaryCta: "ابدأ" }, codeSketch: ["interface Course {}"], testPlan: ["اختبار العزل"], risks: ["الخصوصية"], approvalsRequired: ["اعتماد المالك"] });
